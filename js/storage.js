...
function saveTasks(v){saveJSON(STORAGE_KEYS.tasks,v);if(window.BackendSync)BackendSync.requestPush()}
function getTeam(){return loadJSON(STORAGE_KEYS.team,DEFAULT_TEAM)}
function saveTeam(v){saveJSON(STORAGE_KEYS.team,v);if(window.BackendSync)BackendSync.requestPush()}
function getActivity(){return loadJSON(STORAGE_KEYS.activity,[])}
function saveActivity(v){saveJSON(STORAGE_KEYS.activity,v.slice(0,200));if(window.BackendSync)BackendSync.requestPush()}
function getComments(){return loadJSON(STORAGE_KEYS.comments,{})}
function saveComments(v){saveJSON(STORAGE_KEYS.comments,v);if(window.BackendSync)BackendSync.requestPush()}
