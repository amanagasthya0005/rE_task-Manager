...
const BackendSync = (() => {
  ...
  async function pushAll() {
    if (!configured || suppressPush) return {ok:false,error:new Error("Backend is not configured")};
    try {
      const { data: { session: authSession }, error: authError } = await client.auth.getSession();
      if (authError) throw authError;
      if (!authSession?.user || authSession.user.email !== window.RE_CONFIG.EDITOR_AUTH_EMAIL) {
